...
public class AI {
	...
	public static Vector<Double> narrowMovesFutureMax(Vector<Integer> moves, GameState gameState, Vector<HoleClass> holes, int countdown, int currentDepth, int maxDepth){
		...
		for(int i = 0; i < moves.size(); i++) {
			if(countdown >= 2) {
				...
				double moveResult = AI.fullTurnFutureMax(gameStateAI, moves.get(i), totalHolesInBoard, nextHoles, countdown, currentDepth, maxDepth);
	...
	public static Vector<Double> narrowMovesFutureMin(Vector<Integer> moves, GameState gameState, Vector<HoleClass> holes, int countdown, int currentDepth, int maxDepth){
		Vector<Double> moveValues = new Vector<Double>();
		for(int i = 0; i < moves.size(); i++) {
			moveValues.add(2.0);
		}
		for(int i = 0; i < moves.size(); i++) {
			if(countdown >= 2) {
				...
				int totalHolesInBoard = gameState.boardSize*2 + 2;
				double moveResult = AI.fullTurnFutureMin(gameStateAI, moves.get(i), totalHolesInBoard, nextHoles, countdown, currentDepth, maxDepth);
				//System.out.println("Move Result: " + moveResult);
				moveValues.set(i, moveResult);
			}
		}
	...
	public static double fullTurnFutureMax(GameState gameState, int hole, int totalHolesInBoard, Vector<HoleClass> nextHoles, int countdown, int currentDepth, int maxDepth){
		...
		if(result == true) {
			...
			if(currentMove == gameState.currentMove) {
				...
				Vector<Double> moveQualities = narrowMovesFutureMax(moves, gameState, nextHoles, countdown, currentDepth, maxDepth);
			...
			if(currentDepth < maxDepth) {
				Vector<Integer> moves = possibleMovesFuture(gameState, nextHoles);
				currentDepth++;
				Vector<Double> moveQualities = narrowMovesFutureMin(moves, gameState, nextHoles, countdown, currentDepth, maxDepth);
	...
	public static double fullTurnFutureMin(GameState gameState, int hole, int totalHolesInBoard, Vector<HoleClass> nextHoles, int countdown, int currentDepth, int maxDepth){
		boolean result = movePiecesFuture(gameState, hole, nextHoles);
		if(result == true) {
			stealPiecesFuture(gameState, nextHoles);
			//This determines who will move next depending on where the last piece ends up
