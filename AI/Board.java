...
public class Board extends JPanel implements MouseListener 
{
	...
	static boolean pie_on; //added by drew
	...
	{
		setLayout(null);
		//CHANGE THIS TO CHANGE TIME
		countdown = 5;
		pie_on = false; //added by drew
		pie_choice=false;
	...
	//this adds the holes to the board
		...
		public static Vector<Double> narrowMoves(Vector<Integer> moves, GameState gameState, int countdown){
			...
			for(int i = 0; i < moves.size(); i++) {
				...
				double moveResult = AI.fullTurnFutureMax(gameStateAI, moves.get(i), totalHolesInBoard, nextHoles, countdown, 0, 0);
			...
			while((countdown >= 2) && (currentDepth < 3)) {
				//Vector<HoleClass> nextHoles = holes;
				GameState gameStateAI = new GameState(gameState);
				...
				double moveResult = AI.fullTurnFutureMax(gameStateAI, moves.get(currentNode), totalHolesInBoard, nextHoles, countdown, 0, currentDepth);
		...
		public static void fullTurn(GameState gameState, int hole, int totalHolesInBoard){
			...
			if((gameState.gameOver == 0) && (gameState.currentMove == 2)) {
				...
				System.out.println("Best Move Pruning: " + chosenMove);
			}
		}
