...
public class Board extends JPanel implements MouseListener 
{
	...
	GameState gameState;
	 public static JLabel time_lbl ;
	static int countdown;
	Timer timer=null;
	
	static String vecToString() {
		String concatenate = "";
		for(HoleClass hc: holes) {
			concatenate=concatenate + hc.toString()+"$";
		}
		return concatenate;
	}
	/**
	...
	public Board() 
	{
		if(GameClient.player.equals("2")) my_turn=false;
		setLayout(null);
		countdown = 30;
		pie_choice=false;
		invalid_click=false;
		...
		new Thread(()->{
			while(true) {
				try {
					...
					String tokenzz[] = msg.split("\\&");
                    if(tokenzz[0].equals("MOVE")) {
                    	my_turn=true;
                    	String tokenzzz[] = tokenzz[1].split("\\$");
                    	holes.clear();
                    	for(String token:tokenzzz) {
                    		HoleClass hc=HoleClass.fromString(token);
                    		holes.add(hc);
                    	}
	...
	//this adds the holes to the board
		...
		public static void fullTurn(GameState gameState, int hole, int totalHolesInBoard){
			...
			if(result == true) {
				GameClient.out.println("MOVE&" + vecToString());
